...
function onDateChanged(start, end) {
  ...
  update(); // check campsite status based on selected reservation dates
  // any campsites returned in ${data} is meant to be unavailable

  Stoney.GetCampgroundsStatus("".concat(start.format(TIME_FORMAT), " - ").concat(end.format(TIME_FORMAT)), function (data, status, r) {
    var campsites = document.querySelectorAll('#cg-campsite-list > div'); // loop thru all campsites

    for (var a = 0; a < campsites.length; a++) {
      var campsite = campsites[a]; // DOM element

      var _status = document.querySelector("#".concat(campsite.id, "-status")); // DOM element


      _status.innerHTML = "Checking...";

      _status.classList.remove('bg-danger', 'bg-success');
      ...
      for (var b = 0; b < data.length; b++) {
        ...
        if (row.status == 'paid') {
          ...
          _status.classList.add('bg-warning', 'text-dark');

          _status.innerHTML = "Pending";
        }
...
function onCampersChanged() {
  ...
  var count = Math.max(1, Math.min(12, document.querySelector('#campers-count').value));
...
function update() {
  $('#nights').html("This reservation will be for <span class=\"fw-bold\">".concat(nights, " night").concat(nights == 1 ? '' : 's', "</span>"));
