            ...
            <div class="row justify-content-center my-5">
                <div class="col-md-6">
                    <small class="d-block">Your stay is {{ $nights }} night{{ $nights > 1 ? 's' : '' }} long
                        with {{ sizeof($campers) }} {{ sizeof($campers) == 1 ? 'person' : 'people' }}.</small>
                    <div class="tl">
                        @for ($i = 0; $i < $nights + 1; $i++)
                            <div class="tl-item">{{ date('F j', strtotime($reservation->date_in) + $i * 86400) }}</div>
                        @endfor
                    </div>
                </div>
            </div> <!-- timeline visual -->

            <p>In a few moments you'll receive an email showing this reservation.</p>
